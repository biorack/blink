...
def _construct_massdiff_sparse_matrices(d_spec, max_mz):

    depth = len(d_spec['massdiff_bins_list'])
    dims = np.concatenate([np.full((massdiff_bins.shape[0],), idx) for idx, massdiff_bins in enumerate(d_spec['massdiff_bins_list'])])
    max_id = np.max(d_spec['spec_ids'])
    
    ids_mod = dims * (max_id + 1)
    ids_tiled = np.tile(d_spec['spec_ids'], depth)
    ids_extended = ids_tiled + ids_mod
    
    i_data_tiled = np.tile(d_spec['normalized_intensities'], depth)
    c_data_tiled = np.tile(d_spec['counts'], depth)
    
    mass_diff_bins = np.concatenate(d_spec['massdiff_bins_list'])
...
def _build_matrices_for_network(n_mzis_s1, n_mzis_s2, s1_df, s2_df, tolerance, bin_width, mass_diffs):
    ...
    network_sparse_matrices = {'s1':{'mdi':s1_massdiff_m_i, 'mdc':s1_massdiff_m_c, 'metadata':n_mzis_s1['metadata']},
